...
- **WonderShow Core**（開源 Swift Package）：`.wondershow` 專案格式定義、MediaPipe 側車協議、插件 API
- **WonderShow App**（社群版）：完整錄製工作流、UI、影片合成引擎
...
WonderShow/
├── Sources/
│   ├── WonderShow/              # 核心庫（錄製模型、專案格式）
│   └── WonderShowApp/           # macOS App（Dashboard、錄製、合成）
├── Tests/                       # 單元測試（244+ 項）
├── open-source/
